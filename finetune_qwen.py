...
def get_mask_grad(model, module_name, src_lang_code, tgt_lang_code):
    ...
    retain_index = retain_index.long()
...
if __name__ == "__main__":
    ...
    model = AutoModelForCausalLM.from_pretrained(model_path, device_map='balanced', max_memory=max_memory, trust_remote_code=True)
    writer = SummaryWriter(log_path)

    save_iter = 3500

    module_name_list = [
        'model.layers.27.input_layernorm', "model.layers.27.self_attn.q_proj", "model.layers.27.self_attn.k_proj", "model.layers.27.self_attn.v_proj", 'model.layers.27.self_attn.o_proj', 'model.layers.27.post_attention_layernorm', 'model.layers.27.mlp.gate_proj', 'model.layers.27.mlp.down_proj', 'model.layers.27.mlp.up_proj',
        'model.layers.28.input_layernorm', "model.layers.28.self_attn.q_proj", "model.layers.28.self_attn.k_proj", "model.layers.28.self_attn.v_proj", 'model.layers.28.self_attn.o_proj', 'model.layers.28.post_attention_layernorm', 'model.layers.28.mlp.gate_proj', 'model.layers.28.mlp.down_proj', 'model.layers.28.mlp.up_proj',
        'model.layers.29.input_layernorm', "model.layers.29.self_attn.q_proj", "model.layers.29.self_attn.k_proj", "model.layers.29.self_attn.v_proj", 'model.layers.29.self_attn.o_proj', 'model.layers.29.post_attention_layernorm', 'model.layers.29.mlp.gate_proj', 'model.layers.29.mlp.down_proj', 'model.layers.29.mlp.up_proj',
        'model.layers.30.input_layernorm', "model.layers.30.self_attn.q_proj", "model.layers.30.self_attn.k_proj", "model.layers.30.self_attn.v_proj", 'model.layers.30.self_attn.o_proj', 'model.layers.30.post_attention_layernorm', 'model.layers.30.mlp.gate_proj', 'model.layers.30.mlp.down_proj', 'model.layers.30.mlp.up_proj',
    ]
    # 冻结指定结构之外的其他参数
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            if batch_idx % save_iter == 0 and batch_idx != 0:
                folder_path = save_model_path + "/" + str(batch_idx)
    ...
    folder_path = save_model_path + "/last"
    os.makedirs(folder_path, exist_ok=True)
    model.save_pretrained(folder_path + "/")