...
def get_mask_scale_grad(model, module_name, src_lang_code, tgt_lang_code):
    """返回单一指定结构的mask梯度，保留的梯度按照神经元重要性分数进行缩放
...
def get_all_mask_grad(model, module_name_list, src_lang_code, tgt_lang_code):
    ...
    for i in range(len(module_name_list)):
        ...
        mask_grad = get_mask_scale_grad(model, module_name, src_lang_code, tgt_lang_code)
