...
if __name__ == "__main__":
    ...
    print('lang_pairs:',args.lang_pairs) 
    ...
    # model = BloomForCausalLM.from_pretrained(model_path,device_map='balanced', max_memory=max_memory)
    model = LlamaForCausalLM.from_pretrained(model_path,device_map='balanced', max_memory=max_memory)
    ...
    # ]
    module_name_list = [
        'model.layers.20.input_layernorm', 'model.layers.20.self_attn.q_proj', 'model.layers.20.self_attn.k_proj', 'model.layers.20.self_attn.v_proj', 'model.layers.20.self_attn.o_proj', 'model.layers.20.post_attention_layernorm', 'model.layers.20.mlp.gate_proj', 'model.layers.20.mlp.up_proj', 'model.layers.20.mlp.down_proj',
        'model.layers.21.input_layernorm', 'model.layers.21.self_attn.q_proj', 'model.layers.21.self_attn.k_proj', 'model.layers.21.self_attn.v_proj', 'model.layers.21.self_attn.o_proj', 'model.layers.21.post_attention_layernorm', 'model.layers.21.mlp.gate_proj', 'model.layers.21.mlp.up_proj', 'model.layers.21.mlp.down_proj',
        'model.layers.29.input_layernorm', 'model.layers.29.self_attn.q_proj', 'model.layers.29.self_attn.k_proj', 'model.layers.29.self_attn.v_proj', 'model.layers.29.self_attn.o_proj', 'model.layers.29.post_attention_layernorm', 'model.layers.29.mlp.gate_proj', 'model.layers.29.mlp.up_proj', 'model.layers.29.mlp.down_proj',
        'model.layers.30.input_layernorm', 'model.layers.30.self_attn.q_proj', 'model.layers.30.self_attn.k_proj', 'model.layers.30.self_attn.v_proj', 'model.layers.30.self_attn.o_proj', 'model.layers.30.post_attention_layernorm', 'model.layers.30.mlp.gate_proj', 'model.layers.30.mlp.up_proj', 'model.layers.30.mlp.down_proj',
    ]
    # 冻结指定结构之外的其他参数
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            batch_inputs_pad_tensor = pad_sequence(batch_inputs_tensor_list, batch_first=True, padding_value=model.config.bos_token_id).to("cuda")

            batch_labels_tensor_list = [torch.tensor(lst) for lst in batch["labels"]]
            batch_labels_pad_tensor = pad_sequence(batch_labels_tensor_list, batch_first=True, padding_value=model.config.bos_token_id).to("cuda")
